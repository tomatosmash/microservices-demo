...
def get_data_dim(dataset):
    if dataset == 'SMAP':
        ...
        return 38
    elif dataset == 'boutique':
        # 自动从 dim.txt 或 pkl 文件获取维度数
        dim_file = os.path.join(prefix, 'boutique_dim.txt')
        if os.path.exists(dim_file):
            with open(dim_file, 'r') as f:
                return int(f.read().strip())
        # 回退: 直接从 pkl 文件推断
        pkl_file = os.path.join(prefix, 'boutique_train.pkl')
        if os.path.exists(pkl_file):
            with open(pkl_file, 'rb') as f:
                return pickle.load(f).shape[-1]
        raise ValueError(
            '未找到 boutique 数据集的维度信息，请先运行 boutique_preprocess.py'
        )
    else:
...
def get_data(dataset, max_train_size=None, max_test_size=None, print_log=True, do_preprocess=True, train_start=0,
    ...
    if test_label is not None:
        ...
        print("test set label shape: None (无异常标签)")
